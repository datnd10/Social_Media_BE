...
public interface StoryRepository extends JpaRepository<Story, Integer> {
    ...
    @Query("SELECT s FROM Story s WHERE (s.user.id IN :followings AND s.createdAt > :oneDayAgo)  ORDER BY s.createdAt DESC")
    ...
    @Query("SELECT s FROM Story s WHERE (s.user.id = :userId AND s.createdAt > :oneDayAgo)  ORDER BY s.createdAt DESC")
